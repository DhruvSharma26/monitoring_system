const fs = require('fs');
const code = `const dns = require("dns");
...
async function run() {
    try {
        ...
        console.log(\`Device \${devUid} assigned staff ID: \${assignedStaffId}\`);
        ...
        console.log(\`1. Created Yesterday's Unresolved Alert (\${oldAlert._id}) & Task (\${oldTask._id}) | Status: \${oldAlert.status} / \${oldTask.status}\`);
        ...
        if (checkBeforeOldAlert.status !== "EXPIRED" && checkBeforeOldTask.status !== "EXPIRED") {
            console.log("  [PASS] Yesterday's alert & task are NOT EXPIRED on present date when no new alert has arrived!");
        } else {
            console.error("  [FAIL] Yesterday's alert or task was prematurely EXPIRED!");
            process.exit(1);
        }
        ...
        // TEST CHECK 2: Now that a new alert arrived today, yesterday's alert & task MUST be marked EXPIRED!
        ...
        console.log(\`   Today New Task (\${todayTask._id}) Status: \${todayTask.status} | Staff: \${todayTask.staff}\`);

        if (refreshedOldAlert.status === "EXPIRED" && refreshedOldTask.status === "EXPIRED" && todayTask && todayTask.status === "ASSIGNED") {
            console.log("\n  [PASS] Yesterday's alert & task cleanly marked EXPIRED and a NEW alert card & task were created for today assigned to device staff!");
        } else {
        ...
        await Task.deleteMany({ _id: { $in: [oldTask._id, todayTask._id] } });
...
fs.writeFileSync("C:/Users/admin/Downloads/monitoring_system/scratch/test_user_expiration_requirement.js", code, 'utf8');
